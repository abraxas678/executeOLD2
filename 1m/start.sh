...
tar -xf cd $HOME/tmp/$ts/execute/1m-execute-setup.tar
rm cd $HOME/tmp/$ts/execute/1m-execute-setup.tar -f
mkdir /home/abraxas/myq >/dev/null 2>/dev/null
mkdir /home/abraxas/myq/service >/dev/null 2>/dev/null
mv $HOME/tmp/$ts/execute/1m.sh /home/abraxas/myq/service
sudo mv $HOME/tmp/$ts/execute/1m* /etc/systemd/system
